...
/*   Updated: 2023/02/23 22:12:45 by tlegrand         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_get_fd(t_pipex *cmd_l, char **arg, int argc)
{
	...
	if (access(arg[1], F_OK))
		...
		ft_error("Can't open file (permission denied) : ", arg[1], &cmd_l->err);
	cmd_l->fds[0] = open(arg[1], O_RDONLY | O_CLOEXEC);
	cmd_l->fds[1] = open(arg[n], O_WRONLY | O_TRUNC | O_CLOEXEC | O_CREAT, 420);
	if (cmd_l->fds[1] == -1)
		ft_error("Can't open file (permission denied) : ", arg[n], &cmd_l->err);
}
